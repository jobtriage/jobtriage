...
When('the user updates password with the following credentials using the webUI', (table) => {
  tableData = table.parse().hashes()[0];
  accountPage.updatePassword(tableData.currentPassword, tableData.newPassword, tableData.confirmPassword);
});
...
Then('the user must be able to login with email {string} and password {string}', async (email, password) => {
...
Then('the user should be displayed a popup with message {string}', async (message) => {
  I.waitForElement(accountPage.text.popUp, 5);
  await I.seeElement(accountPage.text.popUp);
  await I.see(message, accountPage.text.popUp);
});

Then('a password mismatch error message {string} should be displayed', (message) => {
